...
public class KakaoApplication {
	...
	@Bean
	CommandLineRunner init(UserJPARepository userRepo, ProductJPARepository productRepo, OptionJPARepository optionRepo){
		return args -> {
			userRepo.saveAll(Arrays.asList(newUser("ssar"), newUser("cos")));
	...
	private User newUser(String username){
		return User.builder()
				...
				.username(username)
				.roles(username.equals("admin") ? Collections.singletonList("ROLE_ADMIN") : Collections.singletonList("ROLE_USER"))
				.build();
	}
